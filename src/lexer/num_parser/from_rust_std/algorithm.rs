...
use super::{
    diy_float::Fp,
    num,
    rawfp::{self, fp_to_float, next_float, prev_float, RawFloat, Unpacked},
    table, Big,
};
use std::cmp::min;
...
mod fpu_precision {
    ...
    pub fn set_precision() -> FPUControlWord {
        let cw = 0u16;

        // Compute the value for the Precision Control field that is appropriate for `T`.
        let cw_precision = match size_of::<f64>() {
            4 => 0x0000, // 32 bits
            8 => 0x0200, // 64 bits
...
pub fn fast_path(integral: &[u8], fractional: &[u8], e: i64) -> Option<f64> {
    ...
    let _cw = fpu_precision::set_precision();
    ...
    if e >= 0 {
        Some(<f64 as RawFloat>::from_int(f) * <f64 as RawFloat>::short_fast_pow10(e as usize))
    } else {
        Some(<f64 as RawFloat>::from_int(f) / <f64 as RawFloat>::short_fast_pow10(e.abs() as usize))
    }
}
...
fn algorithm_r(f: &Big, e: i16, z0: f64) -> f64 {
    ...
    loop {
        ...
        if d2 < y {
            ...
            if m == <f64 as RawFloat>::MIN_SIG && d_negative && d2_double > y {
                z = prev_float(z);
            } else {
                return z;
            }
            ...
            if m % 2 == 0 {
                if m == <f64 as RawFloat>::MIN_SIG && d_negative {
                    z = prev_float(z);
                } else {
                    return z;
                }
...
pub fn algorithm_m(f: &Big, e: i16) -> f64 {
    ...
    let min_sig = Big::from_u64(<f64 as RawFloat>::MIN_SIG);
    let max_sig = Big::from_u64(<f64 as RawFloat>::MAX_SIG);
    loop {
        ...
        if k > <f64 as RawFloat>::MAX_EXP_INT {
            return f64::INFINITY;
        }
        if x < min_sig {
            u.mul_pow2(1);
...
fn quick_start(u: &mut Big, v: &mut Big, k: &mut i16) {
    ...
    let target_ratio = <f64 as RawFloat>::SIG_BITS as i16;
...
fn underflow(x: Big, v: Big, rem: Big) -> f64 {
    ...
    let lsb = bits - <f64 as RawFloat>::SIG_BITS as usize;
    let q = num::get_bits(&x, lsb, bits);
    let k = <f64 as RawFloat>::MIN_EXP_INT + lsb as i16;
    let z = rawfp::encode_normal(Unpacked::new(q, k));
    let q_even = q % 2 == 0;
